...
// Das ist eine Funktion
const add2 = (number1: number, number2: number) => {
    return number1 + number2;
}

const peugeot206CC: Car = {
    id: 1,
...
const allPuntos: Car [] = cars.filter((car) =>
// Die Funktion die wir anwenden wollen
{
...
const allPeugeots: Car [] = cars.filter((car) =>
// Die Funktion die wir anwenden wollen
{
    // Gib alle zurück die Model "Punto" haben
...
const carsAfterDeletion: Car [] = cars.filter((car) =>
// Die Funktion die wir anwenden wollen
{
    // Gib alle zurück die Model "Punto" haben
...
const filmEmpfehlungen: string[] = goodMovies.map((movie) => {
